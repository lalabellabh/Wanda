...
function loadWandaVoices() {
  ...
  const femaleHints = ['female','zira','samantha','victoria','karen','susan','hazel','aria','jenny','libby','sara','sarah','sophie','emma','ava','olivia'];
...
function speak(text) {
  ...
  if (typeof window.WandaVoice === 'function') {
    window.WandaVoice(text, { rate:0.94, pitch:1.10, volume:0.95, voice:wandaFemaleVoice });
    return;
  }
  loadWandaVoices();
  synth.cancel();
  ...
  if (wandaFemaleVoice) utterance.voice = wandaFemaleVoice;
...
function startVoice() {
  ...
  if (recognition) {
    ...
    if (getEl('voiceButton')) getEl('voiceButton').textContent = '🎙 Voice';
    return;
  }
  ...
  recognition.onstart = () => {
    ...
    if (getEl('voiceButton')) getEl('voiceButton').textContent = '🔴 Listening';
  };
...
function startWandaGaze() {
  const stage = getEl('wandaCoreStage');
  const face = getEl('wandaFace');
  const eyes = stage?.querySelectorAll('.orb-eye');
  if (!stage || !face || !eyes?.length) return;
  ...
  const update = () => {
    ...
    const eyeX = `${(currentX * 6).toFixed(2)}px`;
    const eyeY = `${(currentY * 4).toFixed(2)}px`;
    eyes.forEach((eye) => {
      eye.style.setProperty('--eye-x', eyeX);
      eye.style.setProperty('--eye-y', eyeY);
    });
    face.style.transform = `translate(${(currentX * 2.2).toFixed(2)}px, ${(currentY * 1.4).toFixed(2)}px) rotate(${(currentX * 2.2).toFixed(2)}deg)`;
    if (Math.abs(targetX - currentX) > 0.002 || Math.abs(targetY - currentY) > 0.002) raf = requestAnimationFrame(update);
  };
...
document.addEventListener('DOMContentLoaded', () => {
  loadWandaVoices();
  if (synth) synth.addEventListener?.('voiceschanged', loadWandaVoices);
